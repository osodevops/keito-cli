...
async fn login(global: &GlobalFlags, mode: OutputMode) -> Result<(), AppError> {
    ...
    // Store the long-lived API key in config so this CLI works without an
    // interactive keychain. Keyring remains a best-effort additional store.
    ...
    if let Err(e) = ResolvedAuth::store_api_key(&api_key) {
        if mode != OutputMode::Json {
            eprintln!(
                "{} Could not store key in keyring: {e}. Credentials were saved to config.",
                "warning:".yellow().bold()
            );
        }
    }
...
async fn logout(_global: &GlobalFlags, mode: OutputMode) -> Result<(), AppError> {
    let keyring_result = ResolvedAuth::delete_api_key();
    ...
    if mode == OutputMode::Json {
        let out = serde_json::json!({
            ...
            "keyring_credentials_removed": keyring_result.is_ok(),
        });
        println!("{}", serde_json::to_string_pretty(&out).unwrap());
    } else {
        ...
        if let Err(e) = keyring_result {
            eprintln!(
                "{} Could not remove keyring credential: {e}",
                "warning:".yellow().bold()
            );
        }
    }
